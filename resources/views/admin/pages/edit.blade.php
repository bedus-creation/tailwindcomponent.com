    ...
    <div class="w-3/4 bg-white mt-16 px-6 py-12 rounded-lg shadow-lg">
        <form action="{{route('pages.update', $page->id)}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <input value="{{$page->name}}" type='text' name="name" placeholder="Page name would be like Label"
            ...
            <div class="mb-6">
                ...
                <input value="{{$page->title}}" type='text' name="title" placeholder="Enter your Page title"
                    class="w-full px-4 py-2 border rounded-lg text-gray-700 focus:outline-none focus:border-green-500" />
            </div>
            <div class="mb-6">
                <label class="block text-gray-600 font-light mb-2">Page Slug</label>
                <input value="{{$page->slug}}" type='text' name="slug" placeholder="Enter your Page URL"
                    class="w-full px-4 py-2 border rounded-lg text-gray-700 focus:outline-none focus:border-green-500" />
            </div>
            <div class="mb-6">
                <label class="block text-gray-600 font-light mb-2">Sidebar Order</label>
                <input value="{{$page->order}}" type='integer' name="order"
                    placeholder="Enter your Sidebar Listing Order"
                    class="w-full px-4 py-2 border rounded-lg text-gray-700 focus:outline-none focus:border-green-500" />
            </div>
            <div class="mb-2">
            ...
            <div class="w-full">
                <img src="{{$seo->cover}}" class="w-full">
            </div>
            <div class="mb-6">
                <label class="block text-gray-600 font-light mb-2">Page Title</label>
                <input type='text' value="{{$seo->page_title}}" name="page_title"
                    placeholder="Page title would inside title tags"
                    class="w-full px-4 py-2 border rounded-lg text-gray-700 focus:outline-none focus:border-green-500" />
            </div>
            <div class="mb-2">
                ...
                <textarea name="meta_keywords"
                    class="w-full px-3 py-2 text-gray-700 border rounded-lg focus:outline-none  focus:border-green-500"
                    rows="4">{{$seo->meta_keywords}}</textarea>
            </div>
            <div class="mb-2">
